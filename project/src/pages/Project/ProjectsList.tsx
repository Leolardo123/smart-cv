  ...
  return (
    <section>
      ...
      <ul>{items.length===0 && <li className="muted">{t('noItems')}</li>}
        {items.map(p=> (
          <li key={p.id}><Link to={`/projects/${p.id}`}>{p.name}</Link>
            <div className="actions"><Link to={`/projects/${p.id}/edit`}>{t('edit')}</Link>
            <button className="secondary" onClick={()=>del(p.id)}>{t('delete')}</button></div></li>
        ))}
      </ul>
    </section>
  )
