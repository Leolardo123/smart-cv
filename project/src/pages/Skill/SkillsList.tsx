  ...
  return (
    <section>
      ...
      <ul>{items.length===0 && <li className="muted">{t('noItems')}</li>}
        {items.map(s=> (<li key={s.id}><Link to={`/skills/${s.id}`}>{s.name}</Link>
          <div className="actions"><Link to={`/skills/${s.id}/edit`}>{t('edit')}</Link>
          <button className="secondary" onClick={()=>del(s.id)}>{t('delete')}</button></div></li>))}
      </ul>
    </section>
  )
}
